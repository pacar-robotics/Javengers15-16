...
void rampFunction() //ramp, goals
{
	...
	//test for accuracy of calcMove

	//calcMove(5.865,40,BACKWARD,REGULATED);
	 //wait10Msec(10000);
 ...
 //display Shift Values
	...
	eraseDisplay();
	displayTextLine(1,"xShift:%4f", xShift);
	displayTextLine(2,"yShift:%4f", yShift);
	gyroTurn(87,60,UNREGULATED);
	wait1Msec(250);
	//prevent interaction with the GyroSensor reading every 20 ms, causing move inaccuracy

	stopTask(IntegrateGyroHeading);
	calcMove(abs(xShift),70,BACKWARD,REGULATED);
	startTask(IntegrateGyroHeading);
	wait1Msec(250);
	gyroTurn(2,50,REGULATED);
	wait1Msec(250);
	calcMove(65,70,BACKWARD,REGULATED);

//push the robot slowly right next to goal
	calcMove(5,20,BACKWARD,REGULATED);
//too fast and it will bounce off the edge of robot.
	...
	dualMotorTurn(15, 60, CLOCKWISE, REGULATED);
...
task IntegrateGyroHeading(){
	float prevEncoderReading=0.0;
	float currentEncoderReading=0.0;
  ...
  time1[T1]=0;
	while(true){
		...
		currentEncoderReading=nMotorEncoder[LeftWheels];

		deltaDistance=abs(currentEncoderReading-prevEncoderReading)*((PI*DIAMETER)/(4*360));
		prevEncoderReading=currentEncoderReading;

		xShift=xShift+sinDegrees(gyroHeading)*deltaDistance;
		yShift=yShift+cosDegrees(gyroHeading)*deltaDistance;
