...
void processControls()
{
	...
	if(ChooseDriver == MainDriver) // For Controller 1
	{
		...
		if(abs(CTRL1_JOY_LEFT_Y-CTRL1_JOY_RIGHT_Y)>75){
			//check for a dual turn attempt using joysticks
		//this is likely when the joysticks are in opposite directions.
			powerFactor=0.50;
		}
		...
		if(abs(CTRL1_JOY_LEFT_Y) > JOYSTICK_THRESHOLD)
		{
			...
			motor[RightWheels] = (int)((prev_J1_Y1+CTRL1_JOY_LEFT_Y)/2 * powerFactor);
		...
		else
		{
			...
			prev_J1_Y1=CTRL1_JOY_LEFT_Y;



		if(abs(CTRL1_JOY_RIGHT_Y) > JOYSTICK_THRESHOLD)
		{
			motor[LeftWheels] = (int)((CTRL1_JOY_RIGHT_Y+prev_J1_Y2)/2 * powerFactor);
			wait1Msec(100);
		}
		else
		{
